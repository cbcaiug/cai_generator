...
export default function ExportButton({ subject, learners, scores, reverseScores, columnOrder, customColumns }: Props) {
  ...
  const handleExport = async (type: 'all' | 'score' | 'reverse') => {
    setIsExporting(true);
  ...
  return (
    <div className="relative">
      <div className="flex bg-black text-white rounded-full transition-all disabled:opacity-50 disabled:pointer-events-none md:divide-x md:divide-white/20">
        <button
          disabled={isExporting}
          onClick={() => handleExport('all')}
          className="flex items-center gap-2 group w-10 h-10 md:w-auto md:h-auto justify-center md:px-5 md:py-2 p-0 rounded-full md:rounded-l-full md:rounded-r-none shrink-0 hover:bg-zinc-800 active:scale-[0.98]"
        >
          ...
          <span className="hidden md:inline font-medium text-sm">{isExporting ? 'Exporting...' : 'Export to Spreadsheet'}</span>
        </button>
        <button 
          onClick={() => setDropdownOpen(!dropdownOpen)}
          className="hidden md:flex items-center justify-center px-2 hover:bg-zinc-800 rounded-r-full active:scale-[0.98]"
        >
          <ChevronDown size={16} />
        </button>
      </div>
      ...
      {dropdownOpen && (
        <>
          ...
          <div className="absolute right-0 top-full mt-2 w-48 bg-white border border-zinc-200 rounded-xl shadow-lg z-50 overflow-hidden text-sm">
            <button 
              onClick={() => handleExport('score')}
              className="w-full text-left px-4 py-2 hover:bg-zinc-50"
            >
              Export Score Sheet Only
            </button>
            <div className="h-px bg-zinc-100" />
            <button 
              onClick={() => handleExport('reverse')}
              className="w-full text-left px-4 py-2 hover:bg-zinc-50"
            >
              Export Reverse Calc Sheet
            </button>
            <div className="h-px bg-zinc-100" />
            <button 
              onClick={() => handleExport('all')}
              className="w-full text-left px-4 py-2 hover:bg-zinc-50 font-medium"
            >
              Export All Sheets
            </button>
          </div>
